...
const EmployeeList = () => {
    ...
    useEffect(() => {
        const filtered = employees
            .filter(emp => emp.nama.toLowerCase().includes(nameFilter.toLowerCase()))
            .sort((a, b) => {
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            {location.state?.message && (
                    <div className={`mb-4 p-4 rounded-lg ${location.state.messageType === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {location.state.message}
                    </div>
                )}
                {message && (
                ...
                <div className="mb-4 flex justify-between items-center">
                    <Link to="/employees/new" className="bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 flex items-center space-x-2">
                        <FiPlus size={20} />
                ...
                <table className="min-w-full divide-y divide-gray-200 mb-8">
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('no_induk')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Employee ID {sortColumn === 'no_induk' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('nama')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Name {sortColumn === 'nama' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('alamat')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Address {sortColumn === 'alamat' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('tgl_lahir')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Date of Birth {sortColumn === 'tgl_lahir' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('tgl_bergabung')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Join Date {sortColumn === 'tgl_bergabung' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('sisa_cuti')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Remaining Leave {sortColumn === 'sisa_cuti' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                ...
                <div className="flex space-x-8 py-4">
                    ...
                    <div className="w-1/2 bg-white p-4 rounded-lg shadow-md">
                        <h2 className="text-lg font-bold mb-2">First 3 Employees</h2>
                    ...
                    <div className="w-1/2 bg-white p-4 rounded-lg shadow-md">
                        <h2 className="text-lg font-bold mb-2">Latest 3 Employees</h2>
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Employee ID</th>
