...
const CutiList = () => {
    ...
    useEffect(() => {
        const filtered = cutis
            .filter(cuti => cuti.employee?.nama?.toLowerCase().includes(nameFilter.toLowerCase()))
            .sort((a, b) => {
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            {location.state?.message && (
                ...
                <div className="mb-4 flex justify-between items-center">
                    <Link to="/cutis/new" className="bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 flex items-center space-x-2">
                        <FiPlus size={20} />
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('tgl_cuti')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Date {sortColumn === 'tgl_cuti' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('employee.nama')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Employee {sortColumn === 'employee.nama' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('lama_cuti')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Number of Days {sortColumn === 'lama_cuti' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                            <th onClick={() => handleSort('keterangan')} className="cursor-pointer px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Description {sortColumn === 'keterangan' && (sortDirection === 'asc' ? '🔼' : '🔽')}
                            </th>
                        ...
                        {filteredCutis.map(cuti => (
                            <tr key={cuti.id}>
                                ...
                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 flex space-x-4">
                                    <Link to={`/cutis/${cuti.id}`} className="text-indigo-600 hover:text-indigo-900">
                                        <FiEdit size={20} />
                                    </Link>
                                    <button onClick={() => handleDelete(cuti.id)} className="text-red-500 hover:text-red-700">
                                        <FiTrash size={20} />
                                    </button>
                                </td>
                            </tr>
